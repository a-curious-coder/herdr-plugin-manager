...
if [[ "$(current_mode)" == "catalog" ]]; then
  ...
  echo "/ search (esc cancels) · enter install · a run/bind action · s sort [${sort_mode}] · d uninstall · r re-fetch · z zoom preview · ? help · q quit"
else
  echo "Plugins — INSTALLED (tab: browse registry) · green enabled · yellow update · gray disabled · spinner updating"
  echo "/ search (esc cancels) · enter toggle · a run/bind action · u update · i install by name · d uninstall · r refresh · z zoom preview · ? help · q quit"
fi
